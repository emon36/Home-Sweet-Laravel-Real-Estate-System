...
class OrderController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<a href="'.route('seller.order.invoice',$row->id).'"  title="download invoice" data-toggle="tooltip"  data-placement="bottom" class="edit btn btn-primary btn-sm"><i class=" mdi mdi-download"></i></a> ';
                    $btn .= '<a href="'.route('seller.order.confirmation',$row->id).'"  title="send confirmation" data-toggle="tooltip"  data-placement="bottom" class="edit btn btn-primary btn-sm"><i class="mdi mdi-message"></i></a> ';
    ...
    public function create()
    {
        ...
        return view('seller.order.create', ['customers' => $customers, 'projects' => $projects]);
    }
    ...
    {
        return view('seller.order.active_order');
    }
    ...
    public function activeOrders(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ->addColumn('payment_status', function ($data) {
                    if ($data->payment_status == 1) {
                        return '<span class="p-1 bg-success text-dark">Paid</span>';
                    } else {
                        return '<span class="p-1 bg-info text-dark">Due</span>';
                    }
                })

                ->addColumn('action', function($row){
                    $btn = '<a href ="'.route('seller.transaction.create',$row->id).'" title="view" data-toggle="tooltip"  data-placement="bottom" class="btn btn-success btn-sm">Add Payment</a> ';
                    return $btn;
                })
